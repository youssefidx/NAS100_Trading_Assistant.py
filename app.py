...
def detect_support_resistance(df, window=20):
    ...
    # Cluster nearby levels (within 0.2% of price)
    def cluster_levels(levels):
        ...
        return [x['mean'] for x in clusters if len(x['points']) >= 2]  # Require at least 2 touches
...
def plot_price_chart(df, signals):
    ...
    if not signals.empty:
        ...
        fig.add_rows(
            buys.set_index('Datetime')['Price'].rename('Buy Signals'),
            sells.set_index('Datetime')['Price'].rename('Sell Signals')
        )
...
def plot_equity_curve(equity):
    ...
    return st.area_chart(pd.DataFrame({'Equity': equity})
